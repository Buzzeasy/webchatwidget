import { Activity,EventActivity, ConnectionStatus, IBotConnection, Media, MediaType, Message, User } from './directLine';
...
export const events: Reducer<EventState> = (
    state: EventState = {
        avatarLogo: '',
        chatTitle: true,
        chatSubTitle: ''
    ...
    if (action.type == 'Event') {
        switch (action.activity.name) {
            case 'ChangeAvatar':
                var url = action.activity.value;
                if (url.indexOf('http') != 0) {
                    url = 'http://' + url;
                }
                konsole.log("changeavatar state received");
                return {
                    ...state,
                    avatarLogo: url
                }
            case 'SetChatServiceName':
                return {
                    ...state,
                    chatTitle: action.activity.value
                } 
            case 'SetDisplayName':
                konsole.log("setdisplayname state received")
                return {
                    ...state,
                    chatSubTitle: action.activity.value
                }
            default:
                return { ...state }
        }
...
export const format: Reducer<FormatState> = (
    ...
    switch (action.type) {
        ...
        case 'Toggle_Avatar':
            return {
                ...state,
                currentAvatar: action.nextAvatar
            }
        case'Set_Chat_SubTitle':
            return{
                ...state,
                chatSubTitle : action.chatSubTitle
            };
        case 'Set_Window_Open':
            sessionStorage.setItem("hideWindow", (!action.windowOpen).toString());
            return{
                ...state,
...
export type HistoryAction = {
    type: 'Receive_Message' | 'Send_Message' | 'Show_Typing' | 'Receive_Sent_Message' | 'Event'
    activity: Activity
} | {
    ...
    type: "Increment_Unread" | "Reset_Unread"
}
...
export type ChatActions = ShellAction | FormatAction | SizeAction | ConnectionAction | HistoryAction | AdaptiveCardsAction | EventAction;
...
const updateAvatarCSSEpic: Epic<ChatActions,ChatState> = (action$,store) => action$.ofType('Event')
.filter(action => action.activity.name == "ChangeAvatar").do(action=>{
    const state = store.getState();
    let otherAvatar = state.format.currentAvatar =="A" ? "b" : "a";
    document.getElementById('wc-app-root').style.setProperty('--avatar_' + otherAvatar ,state.events.avatarLogo);
    document.getElementById('wc-app-root').style.setProperty('--avatar_url_' + otherAvatar,"url("+state.events.avatarLogo+")");
}).delay(1000)
.map(action =>{
    const state = store.getState();
    let otherAvatar = state.format.currentAvatar =="A" ? "b" : "a";
        
        return {type:"Toggle_Avatar", nextAvatar:otherAvatar.toUpperCase()} as FormatAction;
});

const setChatServiceNameEpic: Epic<ChatActions, ChatState> = (action$, store) => action$.ofType('Event')
    .filter(action => action.activity.name == "SetChatServiceName")
    .map(action => {
        const state = store.getState();
        return { type: "Set_Chat_Title", chatTitle: state.events.chatTitle } as FormatAction;
    });

const updateDisplayNameEpic: Epic<ChatActions, ChatState> = (action$, store) => action$.ofType('Event')
    .filter(action => action.activity.name == "SetDisplayName")
    .map(action => {
        const state = store.getState();
        return { type: "Set_Chat_SubTitle", chatSubTitle: state.events.chatSubTitle } as FormatAction;
    });
...
const updateUnreadMessagesEpic:Epic<ChatActions,ChatState> = (action$,store) => 
    action$.ofType('Receive_Message')
    .map(action =>
        {
            const{ format:{windowOpen}} = store.getState();
            if(!windowOpen){
                return {type:'Increment_Unread'};
            } else{
                return nullAction
            }

        });
const resetUnreadOnShowWindowEpic:Epic<ChatActions,ChatState> = (action$,store) =>
    action$.ofType('Set_Window_Open')
    .map(action =>{
...
const sendTypingEpic: Epic<ChatActions, ChatState> = (action$, store) =>
    ...
    const sizeUpdateEpic: Epic<ChatActions,ChatState> = (action$,store) =>
    action$.ofType('Set_Size')
    .map(_=> {return {type:'Set_Measurements',carouselMargin:undefined} as FormatAction})
...
export const createStore = () =>
    reduxCreateStore(
        ...
        applyMiddleware(createEpicMiddleware(combineEpics(
            setChatServiceNameEpic,
            updateDisplayNameEpic,
            updateAvatarCSSEpic,
            ...
            sizeUpdateEpic,
            resetUnreadOnShowWindowEpic,
            updateUnreadMessagesEpic
        )))
    );
