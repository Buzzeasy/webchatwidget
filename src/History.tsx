...
import { Activity, Message, User, CardActionTypes } from "./directLine";
...
export class HistoryView extends React.Component<HistoryProps, {}> {
  ...
  componentWillUpdate(nextProps: HistoryProps) {
    ...
    if (
      ...
      nextProps.hasActivityWithSuggestedActions
    ) {
    ...
    this.scrollToBottom =
      ...
      ) <= scrollToBottomDetectionTolerance;
  }
  ...
  componentDidUpdate() {
    if (this.props.format.carouselMargin == undefined) {
      ...
      // Subtract the padding from the offsetParent's width to get the width of the content. and the profile width as its missed in measurement
      const maxContentWidth =
        (this.carouselActivity.messageDiv.offsetParent as HTMLElement)
          .offsetWidth -
        (paddedWidth + 50);
  ...
  private autoscroll() {
    ...
    const lastActivity = this.props.activities[
  ...
  render() {
    ...
    if (this.props.size.width !== undefined) {
      if (this.props.format.carouselMargin === undefined) {
        ...
        content = this.props.activities.map(
          (activity, index) =>
            (activity.type !== "message" ||
              activity.text ||
              (activity.attachments && activity.attachments.length)) && (
              <WrappedActivity
...
export const History = connect(
  (state: ChatState) => ({
    ...
    hasActivityWithSuggestedActions: !!activityWithSuggestedActions(
  ...
  {
    setMeasurements: (carouselMargin: number) => ({
  ...
  (stateProps: any, dispatchProps: any, ownProps: any): HistoryProps => ({
    ...
    isSelected: (activity: Activity) =>
      activity === stateProps.selectedActivity,
...
const getComputedStyleValues = (
  el: HTMLElement,
  stylePropertyNames: string[]
) => {
  const s = window.getComputedStyle(el);
  const result: { [key: string]: number } = {};
  stylePropertyNames.forEach(
    name => (result[name] = parseInt(s.getPropertyValue(name)))
  );
  return result;
};
...
export class WrappedActivity extends React.Component<WrappedActivityProps, {}> {
  ...
  render() {
    ...
    switch (this.props.activity.id) {
      ...
      default:
        let sent: string;
        if (this.props.showTimestamp && this.props.activity.type !== "typing")
          sent = this.props.format.strings.timeSent.replace(
            "%1",
            new Date(this.props.activity.timestamp).toLocaleTimeString()
          );
        timeLine = (
          <span>
            &nbsp;
    ...
    const currentAvatar = !!this.props.format
      ? this.props.format.currentAvatar
      : "A"; //this.props.format.currentAvatar;
    ...
    return (
      ...
      >
        <div className={"wc-message-from-" + who + "-layoutwrapper"}>
          {who == "me" && <div className={"wc-message-from-me-profile"} />}
          {who == "bot" && (
            <div className="wc-message-from-bot-profile-root">
              <div
                className={
                  "wc-message-from-bot-profile wc-message-from-bot-profile-a " +
                  (currentAvatar == "B" ? "wc-avatar-transparency" : "")
                }
              />
              <div
                className={
                  "wc-message-from-bot-profile wc-message-from-bot-profile-b " +
                  (currentAvatar == "A" ? "wc-avatar-transparency" : "")
                }
              />
            </div>
          )}
          <div className="wc-message-content-wrapper">
            <div
            ...
            <div className={"wc-message-from wc-message-from-" + who}>
              {timeLine}
            </div>
          </div>
        </div>
